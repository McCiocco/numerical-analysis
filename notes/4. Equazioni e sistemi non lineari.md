...
Quando possiamo essere sicuri della convergenza *globale* di Newton su $[a, b]$? Innanzitutto abbiamo un teorema che ce lo garantisce se sono soddisfatte le seguenti due condizioni:
$$\large \phi(x) \in [a, b] \quad \forall x \in [a, b]$$
$$\large \lvert \phi'(x) \rvert < 1 \quad \forall x \in [a, b]$$
che mi puzza tanto di contrazione. Possiamo dire altro se $\alpha$ è una radice semplice e $f'$ e $f''$ sono monotone nell'intervallo dato. In questo caso il metodo converge $\forall x_{0} \; :\; f(x_{0})f''(x_{0}) < 0$.  
...
Cerchiamo di applicare quanto detto finora ad un caso più generale in cui lavoriamo con una funzione $f : R^m \to R^m$. Per risolvere l'equazione utilizziamo sempre un metodo iterativo di punto fisso di una certa funzione $\phi$. Possiamo allora asserire che se vale la condizione
$$\large \rho(J_{\phi (\alpha)}) < 1$$
Allora il metodo di punto fisso è localmente convergente. Stesso risultato vale se si ha $\lVert J_{\phi(\alpha)} \rVert < 1$, in quanto il raggio spettrale è minore della norma.

##### 4.9 Metodo di Newton-Raphson

Se lo Jacobiano di $f$ è invertibile nell'intervallo di lavoro possiamo utilizzare il *metodo di Newton-Raphson* la cui formulazione è
$$\large \phi(x) = x - J_{f(x)}^{-1}f(x)$$
Si dimostra che se lo Jacobiano è invertibile in $\alpha$ allora si ha convergenza locale superlineare. Nella pratica, per calcolare ogni iterazione si risolve il sistema lineare $J_{f(x_{n})} y = f(x_{n})$ e poi si pone $x_{n+1} = x_{n} - y$. 
Si possono considerare delle varianti di Newton-Raphson che hanno un costo computazionale minore:
- se sostituiamo lo Jacobiano con $J_{f(x_{0})}$ otteniamo il metodo di *Newton semplificato*, molto meno costoso;
- se sostituiamo lo Jacobiano con una matrice diagonale con la stessa diagonale del vero Jacobiano otteniamo il metodo di *Newton-Jacobi*.