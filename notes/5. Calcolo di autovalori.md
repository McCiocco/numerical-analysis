...
Se il vettore di partenza $x_{0}$ non ha una componente nella direzione dell'autovettore associato a $\lambda_{1}$ c'è un problema (almeno a livello teorico) perché la successione convergerà all'autovettore associato a $\lambda_{2}$. Questo a livello teorico: nella pratica si inseriscono errori di calcolo che garantiscono comunque la convergenza a $\lambda_{1}$. Tuttavia come possiamo sapere se questo caso si verifica? Abbiamo due modi:
- se $A$ è normale sappiamo che la base di autovettori è ortogonale, quindi ci basta verificare che $x_{0}^Hu_{1} \neq 0$
- se $A$ non è normale gli autovettori non sono necessariamente ortogonali, quindi il prodotto scalare di sopra non ci fornirà la risposta cercata. Possiamo però utilizzare una proprietà degli autovettori sinistri (vettori per cui $v^HA = \lambda v^H$) per cui sono ortogonali a tutti gli autovettori destri associati ad un diverso autovalore. Pertanto la componente di $x_{0}$ lungo $u_{1}$ sarà data da $c_{1}= \frac{v_{1}^Hx_{0}}{v_{1}^Hu_{1}}$ e affinché sia diversa da 0 basta controllare che $v_{1}^Hx_{0} \neq 0$. 
...
$$\large A_{1} = A - \lambda_{1}u_{1}u_{1}^H$$
è facile verificare che ha come autovalori gli stessi di $A$ ma con $0$ al posto di $\lambda_{1}$. L'autovalore di modulo massimo è ora $\lambda_{2}$ e si può ripetere il procedimento. Nota: questa tecnica di deflazione funziona se $A$ è normale e $u_{1}$ è normalizzato.

##### 5.2 Metodi delle potenze inverse e shiftate

Se vogliamo trovare l'autovalore di modulo minimo di $A$ possiamo sfruttare una semplice osservazione: la matrice inversa $A^{-1}$ ha come autovalori $\lambda_{1}^{-1}, \dots, \lambda_{n}^{-1}$. Pertanto il metodo delle potenze applicato ad $A^{-1}$ convergerà al reciproco dell'autovalore di modulo minimo di $A$. Questo è il metodo delle potenze shiftate.
Più generalmente, possiamo chiederci come fare a trovare l'autovalore più vicino in modulo ad un certo valore $\tau$. Il metodo, detto delle *potenze shiftate*, consiste nell'applicare il metodo delle potenze alla matrice $(A-\tau I)^{-1}$, che avrà autovalori della forma $(\lambda_{i}-\tau)^{-1}$ e dunque saranno di modulo massimo quelli più vicini a $\tau$.

##### 5.3 Metodo di Jacobi
