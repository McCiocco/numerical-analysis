...
Si può fattorizzare ogni matrice in un prodotto che coinvolga una matrice ortogonale $Q$ e una matrice triangolare superiore $R$. Per far questo, tipicamente, si procede nel seguente modo, detto *metodo di Householder*.
...
$$\large v = a_{1} + sign(a_{1})\lVert a_{1} \rVert e_{1}$$
...
Si può dimostrare che vale anche l'identità
