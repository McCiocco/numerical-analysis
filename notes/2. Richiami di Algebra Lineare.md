...
Il Teorema di BC lega il determinante di due matrici $A \in \mathbb{C}^{m \times n}$ e $B \in \mathbb{C}^{n \times m}$ al determinante del loro prodotto $C = AB$, che sarà quadrato di ordine $m$.
Ci sono due casi:
1. $m > n$ 
